#!/usr/bin/env python3
r"""Build/flash the STC-B RTC battery ablation probe.
...
def flash() -> bool:
    ...
    import stcflash
    # 探针 UART 也是 115200；stcflash 发 D 命令必须用同一波特率
    ok = stcflash.flash(str(BASE / "main.hex"), port=PORT, label="rtc-battery-probe", baud=115200)
    print("Flash OK" if ok else "Flash FAIL")
    return ok


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Build/flash STC-B RTC battery probe")
