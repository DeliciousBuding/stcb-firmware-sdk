...
STC15F2K60S2 数据手册第 730 页 EEPROM 选型表：
...
**结论：手册定义 IAP 地址空间为 `0000h..F3FFh`（61 KiB，122 × 512B）；本探针在
`0xE000` 扇区实测读/编程/擦除通过，且 `0xF400` 越界访问被 `CMD_FAIL` 拒绝。
这证明设备自写能力存在，但**没有**逐扇区扫描全片，也没有实现或验证 OTA、传输协议与回滚。**
...
- 这仍是能力验证：探针只证明了**能力存在**，不等于更新链路已设计或验证。
